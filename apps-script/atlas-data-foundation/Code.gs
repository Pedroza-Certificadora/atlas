...
function seedAccModels_() {
  ...
  models.forEach(function(m){
    ...
    const data={NOME:m.name,TIPO:m.type,ASSUNTO:m.subject,HTML:m.html,VARIAVEIS_JSON:JSON.stringify(['NOME','EMPRESA','CPF_CNPJ','TIPO_CERTIFICADO','VALIDADE','MENSAGEM','ASSINATURA','TOKEN_CONVITE']),ATIVO:'SIM',STATUS:'ATIVO',ALTERADO_EM:now,ALTERADO_POR:'SETUP-5.0.4D.1'};
    if(current) updateRow_('MODELOS_EMAIL',m.id,data,'SETUP-5.0.4D.1');
    else appendObject_('MODELOS_EMAIL',Object.assign({ID:m.id,CRIADO_EM:now,CRIADO_POR:'SETUP-5.0.4D.1'},data));
  });
}
