/*
 * Atlas CRM Enterprise Gold - Sprint 4.9.16 FINAL
...
(function (w, d) {
  ...
  function auditView() {
    ...
    w.AtlasAPI.audit("CRM_360_VIEW", { clientId: state.current.id, username: actor(), version: "4.9.16" }).then(function () { notify("Consulta registrada na auditoria."); }).catch(function () { notify("Não foi possível registrar a consulta.", "warning"); });
  }
  ...
  function bind() {
    ...
    id("crm-bulk-export").addEventListener("click", function () {
      ...
      var link = d.createElement("a"); link.href = URL.createObjectURL(new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8" })); link.download = "atlas-clientes-selecionados-4.9.16.csv"; link.click(); URL.revokeObjectURL(link.href);
    });
