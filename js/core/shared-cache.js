...
(function(w){"use strict";
  ...
  function write(name,data){try{if(localStorage.getItem("atlas_security_logout_marker"))return false;if(!w.AtlasAuth||!w.AtlasAuth.session||!w.AtlasAuth.session.getActive())return false;localStorage.setItem(key(name),JSON.stringify({savedAt:Date.now(),data:data}));return true}catch(e){return false}}
